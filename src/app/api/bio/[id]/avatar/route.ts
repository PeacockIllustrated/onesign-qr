...
/**
 * POST /api/bio/[id]/avatar - Upload or replace profile avatar
 *
...
export async function POST(
  ...
  try {
    ...
    // Build storage path: {user_id}/{page_id}/avatar.{ext}
    const ext = file.type.split('/')[1] === 'jpeg' ? 'jpg' : file.type.split('/')[1];
    const storagePath = `${user.id}/${id}/avatar.${ext}`;

    // Delete old avatar if it exists and has a different path
    if (page.avatar_storage_path && page.avatar_storage_path !== storagePath) {
      await supabase.storage
        .from('bio-avatars')
        .remove([page.avatar_storage_path]);
    }

    // Upload new avatar (upsert = overwrite if same path)
    ...
    return NextResponse.json(
      { avatar_url: publicUrl, avatar_storage_path: storagePath },
...
/**
 * DELETE /api/bio/[id]/avatar - Remove the profile avatar
 */
export async function DELETE(
  ...
  try {
    ...
    if (updateError) {
      console.error('Failed to clear avatar path:', updateError.message);
      return NextResponse.json(
